# The Mandy programming language

Mandy is python-like but compiled to concrete values. It uses Python-like syntax
...
- Write integration tests that round-trip small Mandy snippets and check the
  resulting AST shape.
...
- A minimal Rust runtime crate linked into every Mandy executable.
...
- Parse CLI arguments: `mandy build <file>`, `mandy run <file>`, `mandy check <file>`.
...
- `tests/` directory with `.mandy` source files and expected `.stdout` / `.stderr`
  snapshots.
- `tests/fib256.mandy` — 256-bit Fibonacci using `u256`.
- `tests/matmul.mandy` — matrix multiply over `f32`.
- `tests/actors.mandy` — a counter actor demonstrating RPC.
- `tests/channels.mandy` — producer / consumer with a bounded channel.
